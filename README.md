...
A discrete-event digital circuit simulator implemented in Python, based on the classic design presented in Chapter 3.3.4 (*"A Simulator for Digital Circuits"*) of **Structure and Interpretation of Computer Programs (SICP)** by Harold Abelson and Gerald Jay Sussman with Julie Sussman.

The simulator models digital hardware through event-driven simulation where signals travel through interconnected wires, logic gates introduce realistic propagation delays, and an agenda scheduler manages discrete simulation time.
...
Following the design philosophy of SICP, this implementation emphasizes functional programming abstractions and procedural data encapsulation:

1. **Procedural Data Abstraction & Closures (Message Passing)**:
...
4. **Dual Interface**:
   The library exposes both pure functional/procedural functions (`make_wire`, `get_signal`, `set_signal`, `add_to_agenda`, `propagate`) and idiomatic Python object/method access (`wire.get_signal()`, `agenda.propagate()`, `len(queue)`).
...
|---|---|---|
| **`Wire`** | [`circuit_sim.wire`](circuit_sim/wire.py) | Maintains a boolean signal (0 or 1) and a list of action procedures triggered on signal transitions. |
| **`Queue`** | [`circuit_sim.queue`](circuit_sim/queue.py) | FIFO queue data structure used inside time segments. |
| **`TimeSegment`** | [`circuit_sim.agenda`](circuit_sim/agenda.py) | Pairs a discrete integer timestamp with an action queue. |
...
| **Compound Circuits** | [`circuit_sim.circuits`](circuit_sim/circuits.py) | `half_adder`, `full_adder`, `ripple_carry_adder`, `multiplexer`, `demultiplexer`. |
...
sequenceDiagram
    ...
    participant Ag as agenda: Agenda
...
│   ├── agenda.py             # Event schedule & propagate loop
│   ├── gates.py              # Logic gates & PrimitiveFunctionBox
│   ├── circuits.py           # Half-adder, full-adder, ripple-carry, mux/demux
...
│   ├── test_circuits.py      # Circuit logic & timing trace tests
...
```bash
# Run the SICP 3.3.4 Half-Adder Walkthrough
